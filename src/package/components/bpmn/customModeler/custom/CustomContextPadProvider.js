...
let customShapeActions = [];
customShapeAction.map(item=>{
	if(item.type != 'create.group'){
		customShapeActions.push(item)
	}
})
...
ContextPadProvider.prototype.getContextPadEntries = function(element) {
    ...
    // 只有点击列表中的元素才会产生的元素
    if (isAny(businessObject, ['bpmn:StartEvent', 'bpmn:Task', 'bpmn:BusinessRuleTask','bpmn:BoundaryEvent', 'bpmn:ExclusiveGateway','bpmn:ParallelGateway', 'bpmn:DataObjectReference'])) {
        Object.assign(actions, {
            ...batchCreateCustom(customShapeActions, createAction),
	...
	// 只有点击列表中的元素才会产生的元素
	if (isAny(businessObject, ['bpmn:Group','bpmn:BoundaryEvent'])) {
	    Object.assign(actions, {
	        'delete': deleteElement()
	    })
	}
